...
<body class="hold-transition sidebar-mini layout-fixed">
								...
								<div class="row">
									<div class="col-sm-6">
										...
										<div class="form-group">
											<label>상위 카테고리</label> <select class="form-control"
												id="topcategory"></select>
										</div>
									</div>
									<div class="col-sm-6">
										<div class="form-group">
											<label>하위 카테고리</label> <select class="form-control"
												name="subcategory.subcategory_id" id="subcategory"></select>
										</div>
									</div>
								</div>
								<!-- 카테고리 영역 끝 -->
									...
									<div class="input-group">
										...
										<div class="custom-file">
											<input type="file" class="custom-file-input" name="photo"
												id="photo" multiple="multiple"> <label
												class="custom-file-label" for="exampleInputFile">상품
												이미지 선택</label>
										</div>
	...
	// e.target.files에 들어가있는 배열은 읽기 전용 이다.
	// 개발자의 입장에서 삭제가 불가능하다.
		let seletedFile = [];
	...
	for(let i = 0; i < files.length; i++){
		seletedFile[i] = files[i]; //읽기 전용에 들어있었던 각 file들을 우림나의 배열로 옮기자.
		
		const reader = new FileReader();

		reader.onload = function(e){
			let src = e.target.result;
			let productImg = new ProductImg(document.getElementById("preview"),files[i], src, 100, 100);
		};
